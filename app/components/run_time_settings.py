...
run_time_settings = dbc.AccordionItem(
    [  # Accordion content starts here
        ...
        dbc.Row([
                ...
                html.H5(
                    "Directories:", id='directories-mode-header'
                ),
                dbc.Col([
                    dbc.Tooltip(
        ...
        html.Br(),  # Line break for spacing
        html.Div([
        dbc.Row([  # Row for wells input with info symbol and tooltip
            dbc.Col([
            dbc.Tooltip(
        ...
        selection_table,
        html.Br(),  # Line break for spacing
        html.Div([  # Div for displaying the list of wells to be analyzed
            html.P("List of wells to be analyzed:"),
            dbc.Card(
                dbc.CardBody(
                    html.P(id='well-selection-list')  # Placeholder for the dynamic list of selected wells
                )
            )],
            style={'display': 'none'}  # Initially hidden, can be shown based on certain conditions
        )
    ],
